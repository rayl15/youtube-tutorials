...
| [`aws-mcp-server/`](./aws-mcp-server) | Give Claude **read-only** access to AWS. Five tools (`list_ec2_instances`, `find_idle_resources`, `get_monthly_cost`, `scan_security_groups`, `list_s3_buckets`) so Claude can audit an account — find wasted spend and security groups open to `0.0.0.0/0`. **Two locks:** read-only code + a read-only IAM policy that *physically* can't mutate anything. | *link when published* |
